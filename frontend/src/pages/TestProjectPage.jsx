...
function TestProjectPage() {
    ...
    useEffect(() => {
        ...
        const timeout = setTimeout(() => {
            ...
            DATA.subjects.forEach((subj, si) => {
                const angleBase = (si / NUM_SUBJECTS) * Math.PI * 2;
                const armPts = [];
                ...
                subj.topics.forEach((topicName, ti) => {
                    ...
                    const wraps = 1.5;
                    const angle = angleBase + progress * Math.PI * 2 * wraps;
                    const x = Math.cos(angle) * r;
                    const z = Math.sin(angle) * r;

                    const nodeSize = 0.18 + (1 - progress) * 0.12;
                ...
                if (armPts.length > 1) {
                    ...
                    const lm = new THREE.LineBasicMaterial({ color: subj.color, opacity: 0.3, transparent: true });
                    pivot.add(new THREE.Line(lg, lm));
                }
            });
            ...
            function animate() {
                ...
                pivot.rotation.y = rotY + Date.now() * 0.00008;
    ...
    return (
        ...
        <div>
            <div ref={contRef} style={{ position: "relative", width: "100%", height: "580px" }}>
